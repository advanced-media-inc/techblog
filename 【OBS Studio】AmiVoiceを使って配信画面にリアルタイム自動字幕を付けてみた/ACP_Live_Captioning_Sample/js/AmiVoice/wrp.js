var Wrp = function () {
	...
	var wrp_ = {
		...
		segmenterProperties: "postTime=300",
		segmenterPropertiesElement: undefined,
		resultUpdatedInterval: 500,
		...
		connectFailed: undefined,//2021/07/14
	...
	function connect_() {
		if (state_ !== 0) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't connect to WebSocket server (Invalid state: " + state_ + ")");
			return false;
		}
		if (wrp_.connectStarted) wrp_.connectStarted();
		if (!window.WebSocket) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't start feeding data to HTTP server (Unsupported WebSocket class)");
			if (wrp_.disconnectEnded) wrp_.disconnectEnded();
			return true;
		}
		if (wrp_.serverURLElement) wrp_.serverURL = "wss://acp-api.amivoice.com/v1/";
		if (!wrp_.serverURL) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't connect to WebSocket server (Missing server URL)");
			if (wrp_.disconnectEnded) wrp_.disconnectEnded();
			return true;
		}
		try {
			...
			if (wrp_.connectFailed) wrp_.connectFailed("can't connect to WebSocket server (" + e.message + ")");
			if (wrp_.disconnectEnded) wrp_.disconnectEnded();
			return true;
		}
		state_ = 1;
		...
		socket_.onclose = function (event) {
			if (state_ === 1) {
				state_ = 0;
				if (wrp_.connectFailed) wrp_.connectFailed("can't connect to WebSocket server: " + wrp_.serverURL);
				if (wrp_.disconnectEnded) wrp_.disconnectEnded();
				interlock_ = false;
			} else
				if (state_ === 2) {
					state_ = 0;
					if (wrp_.disconnectStarted) wrp_.disconnectStarted();
					if (wrp_.connectFailed) wrp_.connectFailed("disconnected from WebSocket server");
					if (wrp_.disconnectEnded) wrp_.disconnectEnded();
					interlock_ = false;
				} else
					if (state_ === 3) {
			...
			if (tag === 's') {
				if (body) {
					if (state_ === 2) {
						...
						if (wrp_.connectFailed) wrp_.connectFailed("connectFailedcan't start feeding data to WebSocket server (" + body + ")");
						if (wrp_.disconnectStarted) wrp_.disconnectStarted();
						socket_.close();
					} else
						if (state_ === 3) {
							state_ = 23;
							stopCheckIntervalTimeoutTimer_();
							if (wrp_.connectFailed) wrp_.connectFailed("can't start feeding data to WebSocket server (" + body + ")");
				...
				if (tag === 'p') {
					if (body) {
						if (state_ === 2) {
							state_ = 8;
							stopCheckIntervalTimeoutTimer_();
							if (wrp_.connectFailed) wrp_.connectFailed("can't feed data to WebSocket server (" + body + ")");
							if (wrp_.disconnectStarted) wrp_.disconnectStarted();
							socket_.close();
						} else
							if (state_ === 3) {
								...
								if (state_ === 4 || state_ === 5 || state_ === 6) {
									...
									if (state_ === 7) {
										...
										if (state_ === 34 || state_ === 36) {
											state_ = 8;
											stopCheckIntervalTimeoutTimer_();
											if (wrp_.connectFailed) wrp_.connectFaile("can't feed data to WebSocket server (" + body + ")");
											if (wrp_.feedDataPauseEnded) wrp_.feedDataPauseEnded(reason_);
											if (wrp_.disconnectStarted) wrp_.disconnectStarted();
											socket_.close();
										}
					...
					if (tag === 'e') {
						if (body) {
							if (state_ === 2) {
								state_ = 8;
								stopCheckIntervalTimeoutTimer_();
								if (wrp_.connectFailed) wrp_.connectFailed("can't stop feeding data to WebSocket server (" + body + ")");
								if (wrp_.disconnectStarted) wrp_.disconnectStarted();
								socket_.close();
							} else
								if (state_ === 3) {
	...
	function disconnect_() {
		...
		if (state_ !== 2) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't disconnect from WebSocket server (Invalid state: " + state_ + ")");
			return false;
		}
		if (wrp_.disconnectStarted) wrp_.disconnectStarted();
		state_ = 8;
	...
	function feedDataResume_() {
		...
		if (state_ !== 2) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't start feeding data to WebSocket server (Invalid state: " + state_ + ")");
			return false;
		}
	...
	function feedDataResume__(samplesPerSec) {
		...
		if (wrp_.resultUpdatedIntervalElement) wrp_.resultUpdatedInterval = wrp_.resultUpdatedIntervalElement.value * 1000;
		...
		if (wrp_.checkIntervalTimeElement) wrp_.checkIntervalTime = wrp_.checkIntervalTimeElement.value * 1000;
		if (samplesPerSec) {
	...
	function feedData_(data, offset, length) {
		if (state_ !== 5) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't feed data to WebSocket server (Invalid state: " + state_ + ")");
			return false;
		}
		feedData__(data, offset, length);
		return true;
	...
	function feedDataPause_() {
		if (state_ !== 5) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't stop feeding data to WebSocket server (Invalid state: " + state_ + ")");
			return false;
		}
		if (wrp_.feedDataPauseStarted) wrp_.feedDataPauseStarted();
		state_ = 6;
	...
	function issue_() {
		if (!window.XMLHttpRequest) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Unsupported XMLHttpRequest class)");
			return false;
		}
		...
		if (!wrp_.sid) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Missing service id)");
		...
		for (var i = 0; i < wrp_.sid.length; i++) {
			...
			if (!(c >= 0x30 && c <= 0x39 || c >= 0x61 && c <= 0x7A || c >= 0x41 && c <= 0x5A || c === 0x2D || c === 0x5F)) {
				if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Illegal char in service id)");
		...
		if (!wrp_.spw) {
			if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Missing service password)");
		...
		for (var i = 0; i < wrp_.spw.length; i++) {
			...
			if (c < 0x20 || c > 0x7E) {
				if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Illegal char in service password)");
		...
		for (var i = 0; i < wrp_.epi.length; i++) {
			...
			if (c < 0x30 || c > 0x39) {
				if (wrp_.connectFailed) wrp_.connectFailed("can't issue service authorization (Illegal char in pexires in)");
	...
	wrp_.serverURL = "wss://acp-api.amivoice.com/v1/";
