var Recorder = function() {
	...
	var recorder_ = {
		...
		resumeFailed: undefined,//2021/07/14
	...
	function resume_() {
		if (state_ !== -1 && state_ !== 0) {
			if (recorder_.resumeFailed) recorder_.resumeFailed("can't start recording (invalid state: " + state_ + ")");
			return false;
		}
		if (recorder_.resumeStarted) recorder_.resumeStarted();
		if (!window.AudioContext) {
			if (recorder_.resumeFailed) recorder_.resumeFailed("can't start recording (Unsupported AudioContext class)");
			if (recorder_.pauseEnded) recorder_.pauseEnded({code: 2, message: "Unsupported AudioContext class"}, waveFile_);
			return true;
		}
		if (!navigator.mediaDevices) {
			if (recorder_.resumeFailed) recorder_.resumeFailed("can't start recording (Unsupported MediaDevices class)");
		...
		if (recorder_.maxRecordingTimeElement) recorder_.maxRecordingTime = recorder_.maxRecordingTimeElement.value * 60000;
		...
		if (audioSamplesPerSec_ === 0) {
			if (recorder_.resumeFailed) recorder_.resumeFailed("can't start recording (Unsupported sample rate: " + audioContext_.sampleRate + "Hz)");
			reason_.code = 2;
		...
		navigator.mediaDevices.getUserMedia(
			...
			function(error) {
				state_ = 0;
				if (recorder_.resumeFailed) recorder_.resumeFailed("can't start recording (" + error.message + ")");
				reason_.code = 2;
	...
	function pause_() {
		if (state_ !== 2) {
			if (recorder_.resumeFailed) recorder_.resumeFailed("can't stop recording (invalid state: " + state_ + ")");
			return false;
		}
		state_ = 3;
